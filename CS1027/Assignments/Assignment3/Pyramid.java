public class Pyramid {
	...
	private static int[][] index = {{2,2,1,2,1,5,5,5,5},{0,5,5,4,5},{4,4,5,5,0,1,5},
		                            {5,5,4,0,5,5,4,4,4,0,0,5,4,5},
										{1,1,0,1,2,3,3,3,1,1,2,3,4,4,5,3,4,0,5,4,4,5,5,0,5,0,5,4,4,4,0,5,4,3,3,3,4,5,5,0,0,0,5,5,5,0,0},
											{0,5,1,1,1,5,0,0,5,4,3,4,4,4,4,5,0,5,4,4,5,5,4,3,3,3,4,5,5,0,0,0,1,0,0,1,5,4,5,5,4},
											{0,5,5,0,5,0,5,0,1,5,5,1,1,0,1,1,3,2,0,1,3,3,2,3,1,0,0,5,0,5,5,5,5,5,4,4,5,0,5,4,4,0},
	...
	private static int checkPath(String input, DLStack<Chamber> path) {
		if (input.length() == 8 && input.indexOf("map") != -1) {
			...
			if (!curr.isTreasure()) return TEST_FAILED;
			// Check that chambers aer selected according to rules specified in assignment
			while (!path.isEmpty()) {
				next = path.pop();
				if (next != curr.getNeighbour(index[test][i])) return TEST_FAILED;
				++i;
				curr = next;
			}
			// First chamber must be the entrance
			if (!curr.isEntrance()) return TEST_FAILED;
			else return TEST_PASSED;
		}
