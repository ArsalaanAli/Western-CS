...
int main( int argc, char* argv[])
{
	...
	const string fileName = "none.ply";
	...
	float y = MINSIZE;
	float z = MINSIZE;
	int marching_done = false;
	cout << "marching" << endl;
	do{
		
		if(!marching_done){
			int added_verts = marching_cubes(&model, ISOVALUE, STEPSIZE, x, y, z, vertices);
			if(added_verts){
				compute_normals(vertices, normals, added_verts);
			}
