...
Sphere::Sphere(float radius, int sectors, int stacks, bool smooth, int up, bool tex) : interleavedStride(32)
{
    set(radius, sectors, stacks, smooth, up);
        ...
        if (tex) {
            programID = LoadShaders( "GlobeTexture.vertexshader", "GlobeTexture.geoshader", "GlobeTexture.fragmentshader" );
            programID2 = LoadShaders( "GlobeTexture.vertexshader", "GlobeTexture2.geoshader", "GlobeTexture2.fragmentshader" );
...
void Sphere::draw(glm::vec3 lightPos, glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::vec4 color, float alpha) const {
    ...
    // glTexCoordPointer(2, GL_FLOAT, interleavedStride, &interleavedVertices[6]);
        ...
        glVertexAttribPointer(
            ...
            GL_TRUE,                         // normalized?
            interleavedStride,                                // stride
            &interleavedVertices[6]            // array buffer offset
        );
    ...
    M = glm::translate(M, {0.0, 0.0, 0.0});
    glUniformMatrix4fv(glGetUniformLocation(programID2, "M"), 1, GL_FALSE, &MVP[0][0]);
    glUniformMatrix4fv(glGetUniformLocation(programID2, "MVP"), 1, GL_FALSE, &M[0][0]);
    M = glm::mat4(1.0f);

    glDrawElements(GL_POINTS, (unsigned int)indices.size(), GL_UNSIGNED_INT, indices.data());
